...
import io.lyney.notescope.audio.dsp.DspRuntime;
...
public class SpectrumViewModel extends ViewModel {
    ...
    private final MutableLiveData<Float> uiHopSizeInPercent = new MutableLiveData<>();
    ...
    private final MediatorLiveData<Integer> hopSizeInFrames = new MediatorLiveData<>();
    ...
    public SpectrumViewModel(DspConfigRepository repository) {
        ...
        hopSizeInFrames.addSource(uiHopSizeInPercent, value -> computeHopSizeInFrames());
        hopSizeInFrames.addSource(uiFrameSize, value -> computeHopSizeInFrames());

        overlapSliderPosition.addSource(uiFrameSize, value -> computeHopSizeSliderPosition());
        overlapSliderPosition.addSource(uiHopSizeInPercent, value -> computeHopSizeSliderPosition());
        ...
        memoryUsage.addSource(uiHopSizeInPercent, value -> computeMemoryUsage());
        ...
        temporalResolution.addSource(hopSizeInFrames, value -> computeTemporalResolution());
    ...
    private void computeHopSizeInFrames() {
        Integer frameSize = uiFrameSize.getValue();
        Float hopSizeInPercent = uiHopSizeInPercent.getValue();

        if (frameSize != null && hopSizeInPercent != null) {
            hopSizeInFrames.setValue((int) (frameSize*hopSizeInPercent));
        }
    }
    ...
    private void computeHopSizeSliderPosition() {
        ...
        float overlap = 1f - hopSizeInPercent;
    ...
    private void computeWindowTemporalParams() {
        ...
        if (sampleRate != null && frameSize != null) {
            ...
            windowTemporalPrams.setValue( String.format(
                    Locale.getDefault(),
                    "Frequency Resolution: %.02f Hz\nWindow Length: %.02f ms (%3$d Samples)",
                    frequencyRes, windowLen, frameSize
            ));
            return;
        }
    ...
    public void computeMemoryUsage() {
        Integer sampleRate = uiSampleRate.getValue();
        Integer frameSize = uiFrameSize.getValue();
        Integer hopSize = hopSizeInFrames.getValue();
    ...
    public LiveData<Float> getUiHopSizeInPercent() {
        return uiHopSizeInPercent;
    }

    public LiveData<Integer> getHopSizeInFrames() {
    ...
    public void onFftSizeSlide(Float position) {
        ...
        Integer oldHopSize = hopSizeInFrames.getValue();
    ...
    private void loadConfig() {
        ...
        uiHopSizeInPercent.setValue(config.hopSizeInPercent());
    ...
    public void onApplySettings() {
        ...
        if (
            ...
            if (Boolean.TRUE.equals(allowCustomFftSize.getValue())) {
                ...
                uiFrameSize.setValue(frameSize);
            }
    ...
    public void onResetConfig() {
        ...
        uiHopSizeInPercent.setValue(defaultDspConfig.hopSizeInPercent());
