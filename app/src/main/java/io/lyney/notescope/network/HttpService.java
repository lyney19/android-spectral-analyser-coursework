...
import io.lyney.notescope.network.body.RequestBodyFactory;
    ...
    private <T> T parseResponse(
        ...
        try {
            ...
            return switch (contentType) {
                case JSON -> jsonMapper.readValue(responseBody, responseType);
                case XML, SOAP_XML -> parseXml(responseBody, responseType);
                case HTML -> throw new NetworkException(
                        "Server has returned TEXT/HTML. " +
                                "Only String.class is supported for this content type."
                );
            ...
            throw new NetworkException(
                    "An error occurred while parsing data: " + responseBody,
                    e
            );
        }
    }
